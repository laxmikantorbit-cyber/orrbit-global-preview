...
public sealed class PostgresCrmAccountStore : ICrmAccountStore
{
    ...
    public async Task AddAsync(Organisation x,CancellationToken ct=default){await _db.EnsureReadyAsync(ct);const string sql="INSERT INTO businessos_crm.accounts(id,tenant_id,name,legal_name,gstin,display_code,status,roles,contacts,addresses,groups) VALUES(@id,@tenant,@name,@legal,@gstin,@code,@status,CAST(@roles AS jsonb),CAST(@contacts AS jsonb),CAST(@addresses AS jsonb),CAST(@groups AS jsonb))";await using var c=_db.DataSource.CreateCommand(sql);Add(c,x);await c.ExecuteNonQueryAsync(ct);}
    public async Task SaveAsync(Organisation x,CancellationToken ct=default){await _db.EnsureReadyAsync(ct);const string sql="UPDATE businessos_crm.accounts SET name=@name,legal_name=@legal,gstin=@gstin,display_code=@code,status=@status,roles=CAST(@roles AS jsonb),contacts=CAST(@contacts AS jsonb),addresses=CAST(@addresses AS jsonb),groups=CAST(@groups AS jsonb) WHERE tenant_id=@tenant AND id=@id";await using var c=_db.DataSource.CreateCommand(sql);Add(c,x);if(await c.ExecuteNonQueryAsync(ct)==0)throw new InvalidOperationException("Account does not exist.");}
    ...
    private const string Select="SELECT id,tenant_id,name,legal_name,gstin,display_code,status,roles::text,contacts::text,addresses::text,groups::text FROM businessos_crm.accounts";
    private static void Add(NpgsqlCommand c,Organisation x){c.Parameters.AddWithValue("id",x.Id);c.Parameters.AddWithValue("tenant",x.TenantId);c.Parameters.AddWithValue("name",x.Name);Nullable(c,"legal",NpgsqlDbType.Text,x.LegalName);Nullable(c,"gstin",NpgsqlDbType.Text,x.Gstin);Nullable(c,"code",NpgsqlDbType.Text,x.DisplayCode);c.Parameters.AddWithValue("status",(int)x.Status);c.Parameters.AddWithValue("roles",JsonSerializer.Serialize(x.Roles,JsonOptions));c.Parameters.AddWithValue("contacts",JsonSerializer.Serialize(x.Contacts,JsonOptions));c.Parameters.AddWithValue("addresses",JsonSerializer.Serialize(x.Addresses,JsonOptions));c.Parameters.AddWithValue("groups",JsonSerializer.Serialize(x.Groups,JsonOptions));}
    private static Organisation Read(NpgsqlDataReader r){var x=new Organisation(r.GetGuid(0),r.GetGuid(1),r.GetString(2),Str(r,3),Str(r,4),Str(r,5),(OrganisationStatus)r.GetInt32(6));foreach(var role in JsonSerializer.Deserialize<OrganisationRole[]>(r.GetString(7),JsonOptions)??[])x.AddRole(role);foreach(var contact in JsonSerializer.Deserialize<ContactPerson[]>(r.GetString(8),JsonOptions)??[])x.AddContact(contact);foreach(var address in JsonSerializer.Deserialize<OrganisationAddress[]>(r.GetString(9),JsonOptions)??[])x.AddAddress(address);foreach(var group in JsonSerializer.Deserialize<string[]>(r.GetString(10),JsonOptions)??[])x.AddGroup(group);return x;}
...
public sealed class PostgresCrmOpportunityStore : ICrmOpportunityStore
{
    ...
    private const string Select="SELECT id,tenant_id,organisation_id,originating_lead_id,owner_user_id,title,product_service,stage,estimated_value,currency_code,probability_percent,expected_close_date,loss_reason FROM businessos_crm.opportunities";
    private static void Add(NpgsqlCommand c,Opportunity x){c.Parameters.AddWithValue("id",x.Id);c.Parameters.AddWithValue("tenant",x.TenantId);c.Parameters.AddWithValue("org",x.OrganisationId);Nullable(c,"lead",NpgsqlDbType.Uuid,x.OriginatingLeadId);Nullable(c,"owner",NpgsqlDbType.Uuid,x.OwnerUserId);c.Parameters.AddWithValue("title",x.Title);Nullable(c,"product",NpgsqlDbType.Text,x.ProductService);c.Parameters.AddWithValue("stage",(int)x.Stage);c.Parameters.AddWithValue("value",x.Forecast.EstimatedValue);c.Parameters.AddWithValue("currency",x.Forecast.CurrencyCode);c.Parameters.AddWithValue("probability",x.Forecast.ProbabilityPercent);Nullable(c,"close",NpgsqlDbType.Date,x.Forecast.ExpectedCloseDate);Nullable(c,"loss",NpgsqlDbType.Text,x.LossReason);}
    private static Opportunity Read(NpgsqlDataReader r){var forecast=new OpportunityForecast(r.GetDecimal(8),r.GetString(9),r.GetInt32(10),r.IsDBNull(11)?null:r.GetFieldValue<DateOnly>(11));var x=new Opportunity(r.GetGuid(0),r.GetGuid(1),r.GetGuid(2),r.GetString(5),forecast,r.IsDBNull(3)?null:r.GetGuid(3),r.IsDBNull(4)?null:r.GetGuid(4),r.IsDBNull(6)?null:r.GetString(6));var stage=(OpportunityStage)r.GetInt32(7);if(stage==OpportunityStage.Won)x.MarkWon();else if(stage==OpportunityStage.Lost)x.MarkLost(r.IsDBNull(12)?"Lost":r.GetString(12));else if(stage!=OpportunityStage.Discovery)x.MoveTo(stage);return x;}
