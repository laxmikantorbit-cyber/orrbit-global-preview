...
public sealed class Order
{
        ...
        Guid? opportunityId, CommercialSnapshot snapshot)
    {
        ...
        QuoteId = quoteId; OpportunityId = opportunityId; Snapshot = snapshot;
    ...
    {
        if (string.IsNullOrWhiteSpace(paymentId)) throw new ArgumentException("Payment id is required.", nameof(paymentId));
        if (Status == OrderStatus.Activated) throw new InvalidOperationException("Activated order cannot receive another payment.");
        if (Status == OrderStatus.Paid)
        {
            if (string.Equals(PaymentId, paymentId.Trim(), StringComparison.Ordinal)) return;
            throw new InvalidOperationException("Order is already paid by a different payment.");
        }
        if (Status != OrderStatus.PendingPayment) throw new InvalidOperationException("Order cannot be paid in current state.");
