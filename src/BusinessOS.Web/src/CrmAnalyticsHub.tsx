...
import { getCrmDetailedAnalytics, type CrmDetailedAnalytics } from './crmAnalyticsApi'

type View = 'executives' | 'productivity' | 'forecast' | 'accounts' | 'trends'
...
export function CrmAnalyticsHub() {
  ...
  async function refresh() {
    setBusy(true)
    try {
      setData(await getCrmDetailedAnalytics())
  ...
  return <div className="crm2-app crm-advanced-app">
    <aside className="crm2-sidebar">
      ...
      <div className="crm2-sidebar-foot"><strong>SALES ANALYTICS</strong><small>Team · Productivity · Forecast · Customers</small></div>
    </aside>

    <main className="crm2-main">
      <header className="crm2-topbar"><div><span className="crm2-kicker">CRM ANALYTICS</span><h1>{view === 'executives' ? 'Executive performance' : view === 'productivity' ? 'Follow-up & task productivity' : view === 'forecast' ? 'Sales forecast' : view === 'accounts' ? 'Account-wise business' : 'Monthly sales trends'}</h1></div><div className="crm2-top-actions"><button className="crm2-refresh" disabled={busy} onClick={() => void refresh()}>↻ Refresh</button></div></header>
      ...
      <section className="crm2-metrics crm-advanced-metrics">
        ...
        <article><span>Converted leads</span><strong>{totals.converted}</strong><small>Current visible scope</small></article>
      </section>
